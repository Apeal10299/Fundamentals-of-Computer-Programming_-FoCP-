...
group = int(input("How many students? "))
size = int(input("Required group size? "))

total = group // size
left = group % size
if left==1 :
    print(f"There will be {total} groups with {left} student left over.")

else:
    print(f"There will be {total} groups with {left} students left over.")
